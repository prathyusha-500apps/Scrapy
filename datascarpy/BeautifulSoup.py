...
for i in jobs_list:
    ...
    location_experience_private_salary_role = i.find("div",{"class":"cardBody"}).text.strip().replace("\n","").replace("\t","").replace(" ","")
    json_data = [[Title,company,posted_on,location_experience_private_salary_role]]
    print(json_data)
    writer.writerows(json_data)

# while True:
#     try:
#         next = i.find('div', 'arrow arrow-right disabled').get('href')
